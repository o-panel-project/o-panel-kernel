	...
	uint action, enum wl_ext_status status, void *context)
{
	...
	if ((cur_eapol_status >= EAPOL_STATUS_CONNECTING &&
			cur_eapol_status < EAPOL_STATUS_CONNECTED) ||
			(cur_eapol_status >= EAPOL_STATUS_4WAY_START &&
			cur_eapol_status < EAPOL_STATUS_GROUPKEY_M2) ||
			(cur_eapol_status >= EAPOL_STATUS_WSC_START &&
	...
	switch (status) {
		...
		case WL_EXT_STATUS_ADD_KEY:
			...
			wake_up_interruptible(&conf->event_complete);
			IAPSTA_INFO(dev->name, "WPA 4-WAY complete\n");
			break;
		default:
