...
#ifdef WLDWDS
static void
wl_dwds_tput_dump(struct wl_apsta_params *apsta_params, struct wl_dwds_info *dwds_info)
{
	wl_tput_info_t *tput_info = &dwds_info->tput_info;

	if (!(android_msg_level & ANDROID_TPUT_LEVEL))
		return;

	WL_MSG(dwds_info->dev->name, "tx=%3d.%d%d%d Mbps, rx=%3d.%d%d%d Mbps\n",
...
wl_tput_dump(struct wl_apsta_params *apsta_params, struct wl_if_info *cur_if)
{
	...
	if (sta == NULL || (sta->ver != WL_STA_VER_4 && sta->ver != WL_STA_VER_5)) {
		WL_MSG(cur_if->dev->name,
			...
			rssi_buf, tput_info->tput_tx, (tput_info->tput_tx_kb/100)%10,
			(tput_info->tput_tx_kb/10)%10, (tput_info->tput_tx_kb)%10,
			rate/2, (rate & 1) ? ".5" : "",
			tput_info->tput_rx, (tput_info->tput_rx_kb/100)%10,
			(tput_info->tput_rx_kb/10)%10, (tput_info->tput_rx_kb)%10,
...
static void
wl_tput_monitor(struct wl_apsta_params *apsta_params, int ifidx,
	struct wl_tput_info *tput_info)
{
	struct dhd_pub *dhd = apsta_params->dhd;
	dhd_if_t *ifp = NULL;
	...
	if (etype == WLC_E_RESERVED && reason == ISAM_RC_TPUT_MONITOR) {
		...
		for (i=0; i<MAX_IF_NUM; i++) {
			...
			if (tmp_if->dev && tmp_if->ifmode == ISTA_MODE &&
				...
				wl_tput_monitor(apsta_params, tmp_if->ifidx, &tmp_if->tput_info);
				monitor_if[i] = TRUE;
			}
		...
		for (i=0; i<MAX_DWDS_IF_NUM; i++) {
			...
			if (dwds_if->dev) {
				wl_tput_monitor(apsta_params, dwds_if->ifidx, &dwds_if->tput_info);
...
#ifdef WLDWDS
				for (j=0; j<MAX_DWDS_IF_NUM; j++) {
					dwds_if = &apsta_params->dwds_info[j];
					if (dwds_if->dev && tmp_if->bssidx == dwds_if->bssidx) {
						wl_dwds_tput_dump(apsta_params, dwds_if);
					}
				}
#endif /* WLDWDS */
				monitor = TRUE;
	...
	char *dump_buf, int dump_len)
{
	...
	if (dev) {
		...
		chan = wl_ext_get_chan(apsta_params, dev);
		if (chan) {
