...
#define CMD_APSTA_ENABLE		"APSTA_ENABLE"
#define CMD_APSTA_DISABLE		"APSTA_DISABLE"
...
static int
wl_ext_set_channel(struct net_device *dev, struct wl_apsta_params *apsta_params)
{
	s32 _chan = apsta_params->channel;
	chanspec_t chspec = 0;
	...
	if (wf_chspec_valid(chspec)) {
		...
		if (fw_chspec != INVCHANSPEC) {
			if ((err = wldev_iovar_setint(dev, "chanspec", fw_chspec)) == BCME_BADCHAN) {
				...
				printf("%s: channel %d, 0x%x\n", __FUNCTION__, apsta_params->channel, chspec);
		} else {
	...
	s32 bssidx)
{
	...
	if (apsta_params->apmode == AP_MODE) // fix for 43455
		wl_ext_ioctl(dev, WLC_DOWN, NULL, 0, 1);
...
/*
terence 20161127:
wpa_cli driver apsta_init ifname [wlan1] apmode [ap|apsta]
wpa_cli driver apsta_enable ssid [xxx] hidden [y|n] chan [x]
		 amode [open|shared|wpapsk|wpa2psk|wpawpa2psk] emode [none|wep|tkip|aes|tkipaes]
		 key [xxxxx]
wpa_cli driver apsta_disable
*/
static int
wl_ext_apsta_init(struct net_device *dev, char *command, int total_len)
{
	...
	if (apsta_params->action >= ACTION_INIT) {
		ANDROID_ERROR(("%s: don't apsta_init twice\n", __FUNCTION__));
		return -1;
	}
	...
	while (param != NULL) {
		...
		} else if (!strcmp(param, "apmode")) {
			pch = bcmstrtok(&pick_tmp, " ", 0);
			if (pch) {
				if (!strcmp(pch, "ap"))
					apsta_params->apmode = AP_MODE;
				else if (!strcmp(pch, "apsta"))
					apsta_params->apmode = APSTA_MODE;
				else {
					ANDROID_ERROR(("%s: apmode [ap|apsta]\n", __FUNCTION__));
					return -1;
				}
			} else {
				ANDROID_ERROR(("%s: apmode [ap|apsta]\n", __FUNCTION__));
...
static int
wl_ext_apsta_disable(struct net_device *dev)
{
	int ret = 0;
	s8 iovar_buf[WLC_IOCTL_SMLEN];
	struct {
	...
	if (apsta_params->action < ACTION_INIT) {
		ANDROID_ERROR(("%s: please apsta_init apmode [ap|apsta] first\n", __FUNCTION__));
		return -1;
	} else
		apsta_params->action = ACTION_DISABLE;

	printf("%s: apmode=%d\n", __FUNCTION__, apsta_params->apmode);
...
wl_ext_bringup_apsta(struct net_device *dev, struct wl_apsta_params *apsta_params)
{
	...
	apmode_t apmode = apsta_params->apmode;
	...
	wl_ext_set_channel(apdev, apsta_params);
	...
	if (apmode == AP_MODE || apmode == APSTA_MODE) {
		printf("Broadcast SSID: %s\n", apsta_params->hidden ? "OFF":"ON");
...
static int
wl_ext_apsta_enable(struct net_device *dev, char *command, int total_len)
{
	int ret = 0;
	char *pch, *pick_tmp, *param;
	struct wl_apsta_params *apsta_params = &g_apsta_params;
		...
		int *bytes_written)
{
	...
	else if (strnicmp(command, CMD_APSTA_INIT, strlen(CMD_APSTA_INIT)) == 0) {
		*bytes_written = wl_ext_apsta_init(net,
			command + strlen(CMD_APSTA_INIT) + 1, total_len);
	}
	else if (strnicmp(command, CMD_APSTA_ENABLE, strlen(CMD_APSTA_ENABLE)) == 0) {
		*bytes_written = wl_ext_apsta_enable(net,
			command + strlen(CMD_APSTA_ENABLE) + 1, total_len);
	}
	else if (strnicmp(command, CMD_APSTA_DISABLE, strlen(CMD_APSTA_DISABLE)) == 0) {
		*bytes_written = wl_ext_apsta_disable(net);
#endif
	} else
		ret = -1;
